...
doc: # Builds the manpage.
	$(SCDOC) <doc/wpmod.1.scd >wpmod.1

install: # Installs the release binary.
	$(INSTALL) -d \
		$(DESTDIR)$(PREFIX)/$(BINDIR)/ \
		$(DESTDIR)$(PREFIX)/$(MANDIR)/man1/
	$(INSTALL) -pm 0755 wpmod $(DESTDIR)$(PREFIX)/$(BINDIR)/
	$(INSTALL) -pm 0644 wpmod.1 $(DESTDIR)$(PREFIX)/$(MANDIR)/man1/
